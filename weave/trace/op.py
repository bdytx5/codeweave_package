    ...
    *,
    ...
    def op_deco(func: Callable[P, R]) -> Op[P, R]:
        ...
        def create_wrapper(func: Callable[P, R]) -> Op[P, R]:
            ...
            # Capture source location at decoration time (while inspect can still walk func)
            try:
                _source_file = inspect.getfile(func)
                _source_lines, _source_line_start = inspect.getsourcelines(func)
                _source_line_end = _source_line_start + len(_source_lines) - 1
            except (TypeError, OSError):
                _source_file = None
                _source_line_start = None
                _source_line_end = None
            wrapper._source_file = _source_file  # type: ignore
            wrapper._source_line_start = _source_line_start  # type: ignore
            wrapper._source_line_end = _source_line_end  # type: ignore
